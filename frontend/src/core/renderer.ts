...
export class TemplateRenderer {
  ...
  syncTemplate(template: Template, variables: Record<string, string> = {}) {
    ...
    ensureProjectFonts(families).then(loaded => { if (loaded) this.syncTemplate(template, variables); });
  ...
  playTimeline(template: Template, variables: Record<string, string> = {}, onComplete?: () => void) {
    ...
    const targetFrame = infinitePlayback ? maxFrame * 1_000_000 : maxFrame;
    const proxy = { frame: 0 };
