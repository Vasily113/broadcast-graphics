...
import { ProjectFontSelect } from './ProjectFontSelect';
import {
  findKeyframeAtFrame,
...
import {
  TextLayer, RectLayer, ImageLayer, ClockLayer, VideoLayer, VariableBinding, Variable, PositionSizeProp,
...
export function PropertiesPanel() {
        ...
        <Section title="Action">
          <Row>
            ...
            <select
              ...
              onChange={(e) => {
                const command = e.target.value as typeof selectedAction.command;
                if (command === 'setTag') {
                  updateTimelineAction(selectedAction.id, { command, tag: 'Stop', targetDirectorId: null });
                } else {
                  updateTimelineAction(selectedAction.id, {
                    command,
                    tag: null,
                    targetDirectorId: selectedAction.targetDirectorId ?? timeline.directors[0]?.id ?? null,
                  });
                }
              }}
            ...
            >
              ...
              <option value="setTag">Установить tag</option>
            </select>
          </Row>
          {selectedAction.command === 'setTag' ? (
            <Row>
              <Lbl>Tag</Lbl>
              <select
                value={selectedAction.tag ?? 'Stop'}
                onChange={(e) => updateTimelineAction(selectedAction.id, { tag: e.target.value as TimelineActionTag })}
                className="flex-1 bg-surface-700 border border-surface-600 rounded px-2 py-0.5 text-xs text-white min-w-0"
              >
                {TIMELINE_ACTION_TAGS.map((tag) => (
  ...
  if (!baseLayer) {
    return (
      <div className="flex-1 overflow-y-auto text-white px-3 py-4">
        <p className="text-xs text-gray-500">Выберите слой или группу</p>
      </div>
    );
  }
        ...
        return (
          <>
            <Section title="Текст">
              ...
              <Row>
                <Lbl>Шрифт</Lbl>
                <ProjectFontSelect value={l.style.fontFamily} onChange={(v) => updS({ fontFamily: v })} />
              </Row>
              <Row>
                <Lbl>Размер</Lbl>
                <NumInput value={l.style.fontSize} min={1} onChange={(v) => updS({ fontSize: v })} wide />
              </Row>
              <Row>
                <Lbl>Цвет</Lbl>
        ...
        return (
          <>
            ...
            <Section title="Шрифт">
              ...
              <Row>
                <Lbl>Размер</Lbl>
                <NumInput value={l.style.fontSize} min={6} onChange={(v) => updS({ fontSize: v })} wide />
              </Row>
              <Row>
                <Lbl>Цвет</Lbl>
