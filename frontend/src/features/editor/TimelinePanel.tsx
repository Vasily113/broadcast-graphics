  ...
  normalized.directors.forEach((director) => {
    ...
    normalized.actions
      ...
      .forEach((action, index) => {
        const actionLabel = action.command === 'setTag'
          ? `Tag: ${action.tag ?? 'Stop'}`
          : action.command === 'startDirector'
            ? 'Start director'
            : action.command === 'stopDirector'
              ? 'Stop director'
              : `Action ${index + 1}`;
        rows.push({
...
export function TimelinePanel() {
  ...
  const syncLabelsScrollToTracks = useCallback(() => {
    const labelsEl = labelsScrollRef.current;
    const tracksEl = scrollRef.current;
    if (!labelsEl || !tracksEl || syncingScrollRef.current) return;
    if (labelsEl.scrollTop === tracksEl.scrollTop) return;
    syncingScrollRef.current = true;
    tracksEl.scrollTop = labelsEl.scrollTop;
    syncingScrollRef.current = false;
  }, []);

  const syncTracksScrollToLabels = useCallback(() => {
    const labelsEl = labelsScrollRef.current;
    const tracksEl = scrollRef.current;
    if (!labelsEl || !tracksEl || syncingScrollRef.current) return;
    if (labelsEl.scrollTop === tracksEl.scrollTop) return;
    syncingScrollRef.current = true;
    labelsEl.scrollTop = tracksEl.scrollTop;
    syncingScrollRef.current = false;
  }, []);
      ...
      {open && (
        <>
          <div className="px-3 py-2 flex flex-wrap items-center gap-2 border-b border-surface-700 flex-shrink-0">
            ...
            {selectedDirector && (
              <div className="flex items-center gap-1.5 ml-auto pl-2 border-l border-surface-700">
                ...
                <button
                  type="button"
                  disabled={!selectedDirector.loop}
                  onClick={() => {
                    if (selectedDirector.loop) {
                      updateTimelineDirector(selectedDirector.id, { swing: !selectedDirector.swing });
                    }
                  }}
                  className={`px-2 py-1 rounded text-xs font-semibold border ${
                    !selectedDirector.loop
                      ? 'bg-surface-800 text-gray-600 border-surface-700 cursor-not-allowed'
                      : selectedDirector.swing
                        ? 'bg-accent-500 text-white border-accent-400 shadow-inner'
                        : 'bg-surface-700 text-gray-400 border-surface-600 hover:text-white'
                  }`}
                  title={selectedDirector.loop ? 'Ping-pong loop director' : 'Доступно только при включённом Loop'}
                >
                  Swing
                </button>
                <button
          ...
          <div className="flex-1 min-h-0 flex overflow-hidden">
            <div
              className="flex-shrink-0 border-r border-surface-700 flex flex-col min-h-0 text-[11px]"
              style={{ width: LABEL_W }}
            >
              <div className="h-6 flex-shrink-0 border-b border-surface-700 px-2 flex items-center justify-between text-gray-500 bg-surface-850">
                <span>Дорожки</span>
                <button
                  type="button"
              ...
              <div
                ...
                onScroll={syncLabelsScrollToTracks}
              >
              {trackRows.map((row) => (
                <div
                  key={row.rowId}
            ...
            <div
              ...
              onScroll={syncTracksScrollToLabels}
              onMouseDown={(e) => {
