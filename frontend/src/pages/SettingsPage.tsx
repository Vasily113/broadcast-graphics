...
// ── Display modes ─────────────────────────────────────────────────────────────
const DISPLAY_MODES = [
  { id: 'HD1080i50', label: '1920x1080 50i' },
  { id: 'HD1080p50', label: '1920x1080 50p' },
];
...
export function SettingsPage() {
  ...
  const updateField = (
    id: string,
    field: 'name' | 'device_index' | 'display_mode' | 'keyer_mode' | 'show_fps',
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {channels.map((ch, idx) => {
              ...
              const electronCmdLinux = `cd decklink-out && CHANNEL_ID=${ch.id} electron . --no-sandbox`;
                ...
                <div key={ch.id} className="bg-surface-800 border border-surface-700 rounded-xl p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0 space-y-4">
                      ...
                      {/* Row 2: render/output format + keyer mode */}
                      <div className="flex items-end gap-3 flex-wrap">
                        <div className="flex-1 min-w-36">
                          <label className="block text-xs text-gray-500 mb-1">Формат проекта / видеовыхода</label>
                          <select
                        ...
                        <div className={`w-52 ${ch.device_index === -1 ? 'hidden' : ''}`}>
                      ...
                      <label className="inline-flex items-center gap-2 text-xs text-gray-300 cursor-pointer select-none">
                        <input
                          type="checkbox"
                          checked={Boolean(ch.show_fps)}
                          onChange={e => updateField(ch.id, 'show_fps', e.target.checked)}
                          className="w-3.5 h-3.5 accent-indigo-500"
                        />
                        <span>Отобразить FPS</span>
                      </label>
                      ...
                      {/* Electron command */}
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">Запуск decklink-out (Windows CMD)</label>
                        <div className="flex items-center gap-2">
                      ...
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">Запуск decklink-out (Linux / Ubuntu)</label>
                        <div className="flex items-center gap-2">
                          <code className="flex-1 min-w-0 truncate bg-surface-700 border border-surface-600 rounded px-2.5 py-1.5 text-xs text-yellow-400 font-mono">
                            {electronCmdLinux}
                          </code>
                          <CopyButton text={electronCmdLinux} />
                        </div>
                        <p className="text-xs text-gray-600 mt-1">
                          Или перезапустите <code className="bg-surface-700 px-1 rounded">./start-decklink.sh</code> — скрипт поднимет все каналы с SDI.
                        </p>
                      </div>
