...
import { forwardToPlayoutd } from './playoutForward.js';
...
app.ws('/ws/control', (ws) => {
  ...
  ws.on('message', (msg) => {
    ...
    // Native playoutd (DECKLINK_PIPELINE=native-playout)
    forwardToPlayoutd(channelId, command);
...
// Force replay of current on-air state to native playoutd for one channel.
app.post('/api/control/replay/:channelId', (req, res) => {
  const channelId = req.params.channelId || 'default';
  const state = getState(channelId);
  let sent = 0;
  state.forEach((cmd) => {
    forwardToPlayoutd(channelId, cmd);
    sent += 1;
  });
  res.json({ ok: true, channelId, sent });
});
...
// SharedArrayBuffer in Chromium renderer requires cross-origin isolation.
// Enable it only for DeckLink SHM renderer mode to avoid impacting normal preview.
app.use((req, res, next) => {
  if (req.path === '/renderer.html' && req.query.decklinkShm === '1') {
