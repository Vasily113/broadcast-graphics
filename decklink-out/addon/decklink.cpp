...
static channeld::ShmMapping g_prodShm{};
static bool g_prodShmReady = false;
#endif
...
static void copyRgbaRowToBgra(const uint8_t* srcRow, uint8_t* dstRow, int w) {
    for (int x = 0; x < w; ++x) {
...
static void copyRgbaFlippedToBgra(const uint8_t* rgba, uint8_t* dst, int w, int h) {
    for (int y = 0; y < h; ++y) {
        ...
        copyRgbaRowToBgra(srcRow, dstRow, w);
    }
}

// Canvas2D getImageData: top-down RGBA → top-down BGRA.
static void copyRgbaToBgra(const uint8_t* rgba, uint8_t* dst, int w, int h) {
    for (int y = 0; y < h; ++y) {
        const uint8_t* srcRow = rgba + static_cast<size_t>(y) * static_cast<size_t>(w) * 4u;
        uint8_t*       dstRow = dst + static_cast<size_t>(y) * static_cast<size_t>(w) * 4u;
        copyRgbaRowToBgra(srcRow, dstRow, w);
    }
}
...
#ifndef _WIN32
    if (g_prodShmReady) {
        channeld::shmDetach(g_prodShm);
        g_prodShmReady = false;
    }
#endif
...
static Napi::Value ScheduleFrameRgba(const Napi::CallbackInfo& info) {
    ...
    if (info.Length() > 1 && info[1].IsBoolean())
        flipY = info[1].As<Napi::Boolean>().Value();

    if (flipY)
        copyRgbaFlippedToBgra(buf.Data(), g_state.stagingFrame.data(), w, h);
    else
        copyRgbaToBgra(buf.Data(), g_state.stagingFrame.data(), w, h);
    return env.Undefined();
}

#ifndef _WIN32
static Napi::Value ShmProducerAttach(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    if (info.Length() < 3 || !info[0].IsString() || !info[1].IsNumber() || !info[2].IsNumber()) {
        Napi::TypeError::New(env, "shmProducerAttach(name, width, height[, slots])")
            .ThrowAsJavaScriptException();
        return env.Undefined();
    }
    ...
    int slots = 4;
    if (info.Length() > 3 && info[3].IsNumber()) slots = info[3].As<Napi::Number>().Int32Value();

    if (g_prodShmReady) {
        channeld::shmDetach(g_prodShm);
        g_prodShmReady = false;
    }
    g_prodShmReady = channeld::shmAttach(name, w, h, slots, g_prodShm);
    if (g_prodShmReady) {
        fprintf(stderr, "[DeckLink] SHM producer attached: /%s %dx%d slots=%d\n",
                name.c_str(), w, h, slots);
    } else {
        fprintf(stderr, "[DeckLink] SHM producer attach failed: /%s\n", name.c_str());
    }
    return Napi::Boolean::New(env, g_prodShmReady);
}

static Napi::Value ShmProducerPublish(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    if (!g_prodShmReady) return Napi::Boolean::New(env, false);
    if (info.Length() < 1 || !info[0].IsBuffer()) {
        Napi::TypeError::New(env, "shmProducerPublish(buffer[, flipY])")
            .ThrowAsJavaScriptException();
        return env.Undefined();
    }
    auto buf = info[0].As<Napi::Buffer<uint8_t>>();
    ...
    if (info.Length() > 1 && info[1].IsBoolean())
        flipY = info[1].As<Napi::Boolean>().Value();
    const bool ok = channeld::shmPublishRgbaRaw(g_prodShm, buf.Data(), buf.ByteLength(), flipY);
    return Napi::Boolean::New(env, ok);
}

static Napi::Value ShmProducerDetach(const Napi::CallbackInfo& info) {
    (void)info;
    if (g_prodShmReady) {
        channeld::shmDetach(g_prodShm);
        g_prodShmReady = false;
...
static Napi::Value GetSyncStatus(const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    Napi::Object out = Napi::Object::New(env);
    out.Set("externalLocked", Napi::Boolean::New(env, false));
    out.Set("supported", Napi::Boolean::New(env, false));

    if (!g_state.running.load(std::memory_order_acquire) || g_state.deviceCount <= 0) {
        out.Set("reason", Napi::String::New(env, "device-not-running"));
        return out;
    }

    PerDevice& dev = g_state.devices[0];
    if (!dev.output) {
        out.Set("reason", Napi::String::New(env, "no-output"));
        return out;
    }

    BMDReferenceStatus refStatus = 0;
    HRESULT hr = dev.output->GetReferenceStatus(&refStatus);
    if (FAILED(hr)) {
        out.Set("reason", Napi::String::New(env, "get-reference-status-failed"));
        return out;
    }

    out.Set("supported", Napi::Boolean::New(env, true));
    const bool locked = (refStatus & bmdReferenceLocked) != 0;
    out.Set("externalLocked", Napi::Boolean::New(env, locked));
    out.Set("rawStatus", Napi::Number::New(env, static_cast<double>(refStatus)));
    return out;
}
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
    ...
    exports.Set("getSyncStatus",    Napi::Function::New(env, GetSyncStatus));
#ifndef _WIN32
    exports.Set("shmProducerAttach", Napi::Function::New(env, ShmProducerAttach));
    exports.Set("shmProducerPublish", Napi::Function::New(env, ShmProducerPublish));
    exports.Set("shmProducerDetach", Napi::Function::New(env, ShmProducerDetach));
#endif
    return exports;
}
