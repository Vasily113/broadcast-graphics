...
- Windows 10/11 (для SDI через текущий `decklink-out`/аддон)
- Ubuntu (Control/Editor/OBS + SDI Fill+Key через `decklink-out` на Linux)
- Node.js 20+ LTS
...
### Ubuntu быстрый старт (Control + Editor + OBS)
...
- **Renderer (OBS):** `http://localhost:3001/renderer.html?channel=<uuid>`

### Ubuntu SDI Fill+Key (2 канала, DeckLink 8K Pro)

**Требования:**
- Blackmagic Desktop Video for Linux (драйвер + `libDeckLinkAPI.so`)
- SDK в папке `Blackmagic DeckLink SDK 16.0/` (уже в проекте)
- `build-essential`, `python3-setuptools` (для node-gyp)
- Профиль карты: **2 Sub-Devices Full Duplex (2dfd)**
...
**Настройка каналов** (`/settings`):
- Channel 1: `device_index=0`, `keyer_mode=external`, `display_mode=HD1080i50`
- Channel 2: `device_index=1`, `keyer_mode=external`

**Запуск:**
```bash
./start.sh              # backend + frontend
./start-decklink.sh     # 2 процесса Electron → SDI Fill+Key
```

При первом запуске возможна автоперезагрузка Electron после смены профиля на `2dfd`.
