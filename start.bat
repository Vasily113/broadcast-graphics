...
REM      - 3s pause, then 2nd run: profile is already 4dhd, output starts normally
...
start "DeckLink Ch1" cmd /k "cd /d D:\broadcast-graphics\decklink-out && set CHANNEL_ID=399c6610-abd2-46f8-8da4-7c68dfb0aabf&& node_modules\electron\dist\electron.exe . & timeout /t 6 /nobreak > nul & echo [Ch1] Restarting after profile switch... & node_modules\electron\dist\electron.exe ."

REM Small gap between channels to avoid simultaneous profile switches
timeout /t 2 /nobreak > nul

start "DeckLink Ch2" cmd /k "cd /d D:\broadcast-graphics\decklink-out && set CHANNEL_ID=83564e96-01a7-4750-af8a-3ebc124f6ec4&& node_modules\electron\dist\electron.exe . & timeout /t 6 /nobreak > nul & echo [Ch2] Restarting after profile switch... & node_modules\electron\dist\electron.exe ."
...
echo   each Electron will restart automatically after 3 seconds.
echo.
