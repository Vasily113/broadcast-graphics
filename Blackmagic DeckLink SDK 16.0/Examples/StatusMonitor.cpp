...
static std::string formatEthernetStat(INT64_UNSIGNED connectorID, const char* printName)
{
	return "(Port " + std::to_string(connectorID) + ") " + printName;
}

static void printStatistic(IDeckLinkStatistics* deckLinkStats, BMDDeckLinkStatisticID statsId, INT64_UNSIGNED param = 0)
{
	HRESULT        result;
	...
	switch (statsId)
	{
		case bmdDeckLinkStatisticParamEthernetSFPDynamicInfo:
			result = deckLinkStats->GetStringWithParam(statsId, param, &stringVal);
			if (result != S_OK)
				break;
			...
			printf("%-40s %s\n", formatEthernetStat(param, "SFP Dynamic Info:").c_str(),
				stdStringVal.c_str());
			break;

		case bmdDeckLinkStatisticParamEthernetRxPackets:
			result = deckLinkStats->GetIntWithParam(statsId, param, &intVal);
			if (result != S_OK)
				break;

			printf("%-40s %lld\n", formatEthernetStat(param, "Rx packet count:").c_str(), intVal);
			break;

		case bmdDeckLinkStatisticParamEthernetRxDroppedPackets:
			result = deckLinkStats->GetIntWithParam(statsId, param, &intVal);
			if (result != S_OK)
				break;

			printf("%-40s %lld\n", formatEthernetStat(param, "Rx packet drop count:").c_str(), intVal);
			break;

		case bmdDeckLinkStatisticPTPLossOfLock:
			result = deckLinkStats->GetInt(statsId, &intVal);
			if (result != S_OK)
				break;

			printf("%-40s %lld\n", "PTP loss of lock count:", intVal);
			break;

		case bmdDeckLinkStatisticPTPDPLLMarginOfError:
			result = deckLinkStats->GetInt(statsId, &intVal);
			if (result == S_FALSE)
				break;

			printf("%-40s %lld\n", "PTP DPLL Error:", intVal);
			break;

		case bmdDeckLinkStatisticDeviceTemperature:
			result = deckLinkStats->GetInt(bmdDeckLinkStatisticDeviceTemperature, &intVal);
			if (result != S_OK)
				break;
			
			printf("%-40s %u\u00B0C\n", "Device Temperature is:", (unsigned)intVal);
			break;
...
static void printStatus(IDeckLinkStatus* deckLinkStatus, BMDDeckLinkStatusID statusId, INT64_UNSIGNED param = 0)
{
	HRESULT        result;
	INT64_SIGNED   intVal;
	BOOL           boolVal;
	...
	switch (statusId)
	{
		...
		case bmdDeckLinkStatusParamEthernetLink:
		case bmdDeckLinkStatusParamEthernetLinkMbps:
			result = deckLinkStatus->GetIntWithParam(statusId, param, &intVal);
			break;
		case bmdDeckLinkStatusParamEthernetLocalIPAddress:
		case bmdDeckLinkStatusParamEthernetSubnetMask:
		case bmdDeckLinkStatusParamEthernetGatewayIPAddress:
		case bmdDeckLinkStatusParamEthernetPrimaryDNS:
		case bmdDeckLinkStatusParamEthernetSecondaryDNS:
		case bmdDeckLinkStatusParamEthernetSFPStaticInfo:
		case bmdDeckLinkStatusParamEthernetVideoOutputAddress:
		case bmdDeckLinkStatusParamEthernetAudioOutputAddress:
		case bmdDeckLinkStatusParamEthernetAncillaryOutputAddress:
			result = deckLinkStatus->GetStringWithParam(statusId, param, &stringVal);
			if (result == S_OK)
			{
				StringToStdString(stringVal, stdStringVal);
				STRINGFREE(stringVal);
	...
	switch (statusId)
	{
		...
		case bmdDeckLinkStatusParamEthernetLink:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Link State:").c_str(),
				getFourCCName(kEthernetLinkMappings, (BMDEthernetLinkState)intVal));
			break;

		case bmdDeckLinkStatusParamEthernetLinkMbps:
			printf("%-40s %u Mbps\n", formatEthernetStat(param, "Ethernet Link Speed:").c_str(),
				(unsigned)intVal);
			break;
			
		case bmdDeckLinkStatusParamEthernetLocalIPAddress:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Local IP:").c_str(),
				stdStringVal.c_str());
			break;
			
		case bmdDeckLinkStatusParamEthernetSubnetMask:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Subnet Mask:").c_str(),
				stdStringVal.c_str());
			break;
			
		case bmdDeckLinkStatusParamEthernetGatewayIPAddress:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Gateway IP:").c_str(),
				stdStringVal.c_str());
			break;

		case bmdDeckLinkStatusParamEthernetPrimaryDNS:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Primary DNS IP:").c_str(),
				stdStringVal.c_str());
			break;

		case bmdDeckLinkStatusParamEthernetSecondaryDNS:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Secondary DNS IP:").c_str(),
				stdStringVal.c_str());
			break;
		...
		case bmdDeckLinkStatusParamEthernetVideoOutputAddress:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Video Output Address:").c_str(),
				stdStringVal.c_str());
			break;

		case bmdDeckLinkStatusParamEthernetAudioOutputAddress:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Audio Output Address:").c_str(),
				stdStringVal.c_str());
			break;

		case bmdDeckLinkStatusParamEthernetAncillaryOutputAddress:
			printf("%-40s %s\n", formatEthernetStat(param, "Ethernet Ancillary Output Address:").c_str(),
				stdStringVal.c_str());
			break;
		...
		case bmdDeckLinkStatusParamEthernetSFPStaticInfo:
			...
			printf("%-40s %s\n", formatEthernetStat(param, "SFP Static Info:").c_str(),
				stdStringVal.c_str());
			break;
...
public:
	...
	HRESULT STDMETHODCALLTYPE Notify(BMDNotifications topic, INT64_UNSIGNED param1, INT64_UNSIGNED param2)
	{
		...
		printStatus(m_deckLinkStatus, statusId, param2);
...
static void displayHelp()
{
	...
	printf("Usage: StatusMonitor [-d <device id>] [-s <stats interval>] [-h/?]\n\n");
	printf("Options:\n");
	...
	printf("    -s <stats interval>: (default = off)\n");
	printf("       0: Display statistics once\n");
	printf("       n: Display statistics with an interval of n seconds\n");
...
int main(int argc, const char * argv[])
{
	...
	IDeckLinkStatistics*            deckLinkStats        = NULL;
	IDeckLinkNotification*          deckLinkNotification = NULL;
	IDeckLinkProfileAttributes*     deckLinkAttributes   = NULL;
	NotificationCallback*           notificationCallback = NULL;
	int                             deckLinkIndex        = 0;   // Use first device as default
	STRINGOBJ                       deckLinkDisplayName;
	INT64_SIGNED                    deckLinkEthernetConnectorCount = 0;
	HRESULT                         result               = E_FAIL;
	int                             statisticsPollPeriod = -1;  // Don't poll statistics by default
	...
	for (int argIdx = 1; argIdx < argc; ++argIdx)
	{
		...
		if (arg == "-d" && (argIdx + 1 < argc))
		{
			deckLinkIndex = atoi(argv[++argIdx]);
		}
		else if (arg == "-s" && (argIdx + 1 < argc))
		{
			statisticsPollPeriod = atoi(argv[++argIdx]);
		}
		else
		{
			displayHelp();
	...
	// Obtain the statistics interface
	result = deckLink->QueryInterface(IID_IDeckLinkStatistics, (void**)&deckLinkStats);
	if (result != S_OK)
	{
		fprintf(stderr, "Could not obtain the IDeckLinkStatistics interface - result = %08x\n", result);
		goto bail;
	}
	...
	{
		fprintf(stderr, "Failed to get the DeckLink device's ethernet connector count - result = %08x\n", result);
		goto bail;
	}
	...
	printStatus(deckLinkStatus, bmdDeckLinkStatusAncillaryInputSignalLocked);
	...
	if (statisticsPollPeriod >= 0)
	{
		// Print initial statistics values
		printStatistic(deckLinkStats, bmdDeckLinkStatisticDeviceTemperature);
		printStatistic(deckLinkStats, bmdDeckLinkStatisticPTPLossOfLock);
		printStatistic(deckLinkStats, bmdDeckLinkStatisticPTPDPLLMarginOfError);
	}
	
	// Print Ethernet status values
	for (int64_t connectorID = 0; connectorID < deckLinkEthernetConnectorCount; ++connectorID)
	{
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetLink, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetLinkMbps, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetLocalIPAddress, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetSubnetMask, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetGatewayIPAddress, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetPrimaryDNS, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetSecondaryDNS, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetSFPStaticInfo, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetVideoOutputAddress, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetAudioOutputAddress, connectorID);
		printStatus(deckLinkStatus, bmdDeckLinkStatusParamEthernetAncillaryOutputAddress, connectorID);
		
		if (statisticsPollPeriod >= 0)
		{
			// Print initial Ethernet statistics values
			printStatistic(deckLinkStats, bmdDeckLinkStatisticParamEthernetSFPDynamicInfo, connectorID);
			printStatistic(deckLinkStats, bmdDeckLinkStatisticParamEthernetRxPackets, connectorID);
			printStatistic(deckLinkStats, bmdDeckLinkStatisticParamEthernetRxDroppedPackets, connectorID);
		}
	}
...

	{
		...
		if (statisticsPollPeriod > 0)
		{
			// Create thread to poll statistics at interval set with command line argument
			statsThread = std::thread(
				[&terminate, deckLinkStats, deckLinkEthernetConnectorCount, statisticsPollPeriod]()
				{
					auto statisticsPollPeriodSeconds = std::chrono::seconds(statisticsPollPeriod);

					auto fut = terminate.get_future();
					while (fut.wait_for(statisticsPollPeriodSeconds) != std::future_status::ready)
					{
						printStatistic(deckLinkStats, bmdDeckLinkStatisticDeviceTemperature);
						printStatistic(deckLinkStats, bmdDeckLinkStatisticPTPLossOfLock);
						printStatistic(deckLinkStats, bmdDeckLinkStatisticPTPDPLLMarginOfError);
...
bail:
	...
	// Release the statistics interface
	if (deckLinkStats)
		deckLinkStats->Release();
